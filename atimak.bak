...
Proposition quotient_plus : forall (X : Ring) (a : Ensemble (number X)) (i : ideal a) (p : number X -> number X -> number X) (q : QuotientRingNumber X a) (r : QuotientRingNumber X a),
...
Definition quotient := (fun (X : Ring) (a : Ensemble (number X)) (pi : ideal a) =>
  let T := QuotientRingNumber X a in
  let c := (fun
             (p : number X -> number X -> number X)
             (q : T)
             (r : T)
               => (q) : T) in
  mkRing
