...
Definition ideal := (fun (X : Ring) (a : Ensemble (number X)) => (forall x y, a y -> a (mult x y)) /\ Inhabited (number X) a).
...
Definition QuotientRingProp := (fun (X : Ring) (a : Ensemble (number X)) t => exists k : number X, t = (fun m : number X => exists l, a l /\ m = mult k l)).

Definition QuotientRingNumber := (fun (X : Ring) (a : Ensemble (number X)) => {t | QuotientRingProp X a t}).

(*
Proposition quotient_plus : forall (X : Ring) (a : Ensemble (number X)) (i : ideal a) (q : QuotientRingNumber X a) (r : QuotientRingNumber X a),
                                 QuotientRingProp X a (fun v => exists s t, (proj1_sig q) t /\ (proj1_sig r) s /\ v = plus t s).
Proof.
intros.
destruct q.
destruct r.
unfold QuotientRingProp in *.
destruct q.
destruct q0.
unfold proj1_sig.
rewrite e.
...
Definition quotient_plus := (fun (X : Ring) (a : Ensemble (number X)) (i : ideal a) => ).

Definition quotient := (fun (X : Ring) (a : Ensemble (number X)) (i : ideal a) =>
  let T := QuotientRingNumber X a in
  mkRing
  T
  (plus)
  (mult)
